...
total_per_topic = pd.DataFrame(df[df['task'] != '0'].groupby('task')['pomodoros'].sum())
...
fig_total = px.bar(total_per_topic, x = 'task', y='pomodoros',
                   ...
                   color="task")
fig_total.update_layout(font_color="#606060")

#Monthly Pomodoros per topic for second chart
monthly_per_topic = pd.DataFrame(df[df['task'] != '0'].groupby(['month','task'])['pomodoros'].sum())
...
fig_monthly_per_topic.update_layout(font_color="#606060")

#Data for monthly graph in callback below
...
MONTH_SELECTION = by_day['month_name'].unique()

####################App Layout Section####################
app.layout = html.Div(id='page-container', children=[html.Div(className="containerHeader",
    children=[html.Div(className="logoContainer",
                 children=[html.Img(className="myLogo",
                    src='assets/pomo_new.png')]),
            html.Div(className="headerTitleContainer",
                 children=[html.H2("My Pomodoro Dashboard")])]),
                    html.Div(className="summaryContainer",
                    children=[
                        html.Div(className="statsContainer",
                            children=[html.Div(className="summaryValue",
    ...
    html.Div(className="containerCharts",
            children=[html.Div(className="topLeft chartsGeneralSettings",
                children=[dcc.Graph(id='total-graph',
                    figure=fig_total)
                ]),
                html.Div(className="topRight chartsGeneralSettings",
                children=[dcc.Graph(id='monthly-facet-graph',
                    figure=fig_monthly_per_topic)]),
                    html.Div(className="bottomLeft chartsGeneralSettings",
                        children=[dcc.Graph(id='daily-per-month'),
                            html.Div(className="controlLabel",
                                children=["Select Month"]),
                                    html.Div([dcc.Dropdown(id='y-axis-option',
                                    options=[{'label': i, 'value': i} for i in MONTH_SELECTION],
                                    value='January')],
                                        style={'width': '40%', 'display': 'inline-block', 'margin-left': '2%','color':'#FA4F56'}
                                    ),
                        ]),
